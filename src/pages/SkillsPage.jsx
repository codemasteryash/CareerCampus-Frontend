...
export default function SkillsPage() {
  ...
  const handleRemove = async (skillId) => {
    ...
    try {
      ...
      showAlert('success', 'Skill removed from your profile.')
    } catch {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {/* Alert */}
      {alert && (
        <div className={`px-4 py-3 rounded-xl border text-sm flex items-center gap-2 transition-all ${
          alert.type === 'success'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        {/* Left — Catalogue */}
        <div className="lg:col-span-3 bg-[#0f1623] border border-[#1e2d45] rounded-2xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-[#64748b] tracking-widest uppercase">
              Skill Catalogue
            </span>
            <span className="text-[10px] text-[#475569]">{filteredSkills.length} skills</span>
          </div>
          ...
          <div className="space-y-2">
            <input
              ...
              className="w-full px-3 py-2 rounded-xl bg-[#141d2e] border border-[#1e2d45] text-white text-sm placeholder-[#374151] focus:outline-none focus:border-blue-500/60 focus:ring-1 focus:ring-blue-500/30 transition-all"
            />
                ...
                <button
                  ...
                  className={`text-[10px] font-semibold px-2.5 py-1 rounded-full border transition-all ${
                    categoryFilter === cat
                      ...
                      : 'border-[#1e2d45] text-[#475569] hover:text-[#64748b] hover:border-[#2e4060]'
                  }`}
                >
                  {cat}
                </button>
          ...
          <div className="space-y-1.5 max-h-[420px] overflow-y-auto pr-1 scrollbar-thin">
            {filteredSkills.length === 0 ? (
              <div className="text-center py-10 text-[#475569] text-sm">No skills match your search.</div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <div className={`w-1.5 h-1.5 rounded-full ${alreadyAdded ? 'bg-blue-400' : 'bg-[#374151]'}`} />
                    <div>
                      <span className="text-sm text-white font-medium">{skill.name}</span>
                      {skill.category && (
                        <span className="text-[10px] text-[#475569] ml-2">{skill.category}</span>
                      )}
                    </div>
                  </div>
                  {alreadyAdded ? (
                    <span className="text-[10px] text-blue-400 font-semibold">Added ✓</span>
                  ) : (
                    <button
                      onClick={() => setSelectedSkill(String(skill.id))}
                      className={`text-[10px] font-semibold px-2.5 py-1 rounded-lg border transition-all ${
                        selectedSkill === String(skill.id)
                          ? 'bg-blue-600 border-blue-600 text-white'
                          : 'border-[#2e4060] text-[#64748b] hover:text-blue-400 hover:border-blue-500/40'
                      }`}
                    >
                      {selectedSkill === String(skill.id) ? 'Selected' : 'Select'}
                    </button>
                  )}
                </div>
              )
          ...
          >
            {adding ? 'Adding...' : selectedSkill ? `Add with ${proficiency.toLowerCase()} proficiency →` : 'Select a skill above to add'}
          </button>
        </div>

        {/* Right — My Skills */}
        <div className="lg:col-span-2 bg-[#0f1623] border border-[#1e2d45] rounded-2xl p-5 space-y-4">
          <span className="text-xs font-semibold text-[#64748b] tracking-widest uppercase">
            Your Profile
          </span>
          ...
          {mySkills.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 gap-3 text-center">
              ...
              <p className="text-xs text-[#475569]">
                Select skills from the catalogue or upload your resume for automatic extraction.
              </p>
            </div>
          ) : (
            <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
              {mySkills.map(skill => (
                <div
                  key={skill.skillId}
                  className="flex items-center justify-between px-3.5 py-3 rounded-xl bg-[#141d2e] border border-[#1e2d45] hover:border-[#2e4060] transition-all group"
                >
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-white">{skill.skillName}</p>
                    <div className="flex items-center gap-1.5">
                      <Badge
                        text={skill.proficiencyLevel}
                        style={PROFICIENCY_COLORS[skill.proficiencyLevel]}
                      />
                      <Badge
                        text={skill.source === 'RESUME_PARSED' ? 'Resume' : 'Manual'}
                        style={SOURCE_COLORS[skill.source]}
                      />
                    </div>
                  </div>
                  <button
                    ...
                    className="text-[#374151] hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 text-lg leading-none disabled:opacity-50"
                    title="Remove skill"
                  >
          ...
          {/* Legend */}
          <div className="pt-3 border-t border-[#1e2d45] space-y-1.5">
            <p className="text-[10px] font-semibold text-[#475569] tracking-widest uppercase">Legend</p>
            <div className="flex flex-wrap gap-1.5">
